...
async def on_message(message):
    channel = client.get_channel('519074419839336479')
...
async def mute(ctx, member: discord.Member):
    ...
    if member is None:
      ...
      for channel in member.server.channels:
        if channel.name == 'masterpremium-log':
...
async def botinvite(ctx):
    ...
    await client.say('https://discordapp.com/api/oauth2/authorize?client_id=520664286410833921&permissions=8&scope=bot NOTE: THIS IS PREMIUM BOT NOT A FREE ')
...
async def rolesetup(ctx):
    ...
    await client.create_role(author.server, name="Co-owner", permissions=admin_perms)
    await client.create_role(author.server, name="Senior Moderator", permissions=mod_perms)
    await client.create_role(author.server, name="Senior Admin")
